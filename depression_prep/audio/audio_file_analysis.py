...
def max_min_values(current_directory, win_size, hop_size, audio_paths,
                   on_off_times, mode_for_background, config):
    """
    ...
    for iterator, filename in enumerate(audio_paths):
        ...
        orig_data = audio_data.copy()
        if config.EXPERIMENT_DETAILS['Augmentation'] is not None:
            if config.EXPERIMENT_DETAILS['Augmentation'] == 'noise':
                augment = Compose([AddGaussianSNR(min_snr_in_db=5, max_snr_in_db=20, p=1)])
                len_orig = len(audio_data)
                audio_data = augment(samples = audio_data, sample_rate = 16000)
                len_noise = len(audio_data)
                print('Noise Perturb change audio length by {}'.format(len_noise/len_orig))
                print(len_orig)
                print(len_noise)
                assert len_orig == len_noise 
            elif config.EXPERIMENT_DETAILS['Augmentation'] == 'speed':
                aug = naa.SpeedAug(zone = (0,1), factor = (0.9,1.1))
                len_orig = len(audio_data)
                audio_data = aug.augment(audio_data)
                len_sp = len(audio_data)
                print('Speed Perturb Factor is {}'.format(len_sp/len_orig))
                #length mismatch
            elif config.EXPERIMENT_DETAILS['Augmentation'] == 'vtlp':
                aug = naa.VtlpAug(sampling_rate = sample_rate,zone = (0,1))
                import pdb
                pdb.set_trace()
                len_orig = len(audio_data)
                audio_data = aug.augment(audio_data)
                 
                len_vtlp = len(audio_data)
                print('VTLP change audio length by {}'.format(len_vtlp/len_orig))
                print(len_orig)
                print(len_vtlp)
                if len_vtlp != len_orig:
                    import pdb
                    audio_data = cv2.resize(audio_data,(1,len_orig),interpolation=cv2.INTER_NEAREST)
                    audio_data = audio_data[:,0]
                    if len_orig != len(audio_data):
                        print('Still not same')
                #length mismatch  
            elif config.EXPERIMENT_DETAILS['Augmentation'] == 'pitch':
                aug = naa.PitchAug(sampling_rate = sample_rate,zone = (0,1),factor = (-3,3))
                len_orig = len(audio_data)
                import pdb
                audio_data = aug.augment(audio_data)
                len_pitch = len(audio_data)
                print('Pitch Perturb change audio length by {}'.format(len_pitch/len_orig))
                print(len_orig)
                print(len_pitch)
                if len_pitch != len_orig:
                    import pdb
                    audio_data = cv2.resize(audio_data,(1,len_orig),interpolation=cv2.INTER_NEAREST)
                    audio_data = audio_data[:,0]
                    if len_orig != len(audio_data):
            ...
            elif config.EXPERIMENT_DETAILS['Augmentation'] == 'volume':
                aug = naa.LoudnessAug(zone = (0,1),factor = (0.5,2))
                len_orig = len(audio_data)
                audio_data = aug.augment(audio_data)
                len_volume = len(audio_data)
                print('Volume change audio length by {}'.format(len_volume/len_orig))
                print(len_orig)
                print(len_volume)
            elif config.EXPERIMENT_DETAILS['Augmentation'] == 'noise_volume':
                aug_indicator = np.random.randint(2)
                len_orig = len(audio_data)
                if aug_indicator == 0:
                    print('noise')
                    augment = Compose([AddGaussianSNR(min_snr_in_db = 5, max_snr_in_db = 20, p = 1)])
                    audio_data = augment(samples = audio_data, sample_rate = 16000)
                elif aug_indicator == 1:
                    print('volume')
                    aug = naa.LoudnessAug(zone = (0,1), factor = (0.5,2))
                    audio_data = aug.augment(audio_data)
                else:
                    assert 0 == 1
                len_aug = len(audio_data)
                print('Noise/Volume change audio length by {}'.format(len_aug / len_orig))
                assert len_orig == len_aug
        #pdb.set_trace()
        ...
        #for librispeech
        folder_number = int(folder_name[0:-2])
...
def create_database(labels, sample_rate, total_windows_in_file_max, max_value,
                    ...
                    freq_bins, main_logger, whole_train, config,gender='-'):
    """
...
def process_organise_data(main_logger,
    ...
    if config.GENDER:
        ...
        num_samples_feature = create_database(labels, sample_rate,
                                              ...
                                              features_exp, win_size,
                                              hop_size, svn, freq_bins,
                                              main_logger, whole_train, config)
...
def startup():
    ...
    if not os.path.exists(workspace):
        ...
        print('Folder {} created'.format(workspace))
