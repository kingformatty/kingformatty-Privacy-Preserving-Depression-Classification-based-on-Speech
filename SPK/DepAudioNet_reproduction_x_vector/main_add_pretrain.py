...
from exp_run.models_pytorch import DepAudioNet as CustomMel
from exp_run.CPC_model import CDCK2 
...
def create_model():
    ...
    if config.EXPERIMENT_DETAILS['FEATURE_EXP'] == 'mel':
        model = CustomMel(config)
        if config.EXPERIMENT_DETAILS['model'] == 'CPC':
            model = CDCK2()
...
def setup(current_dir, model_dir, data_type='', path_to_logger_for_test=None):
    ...
    if data_type == 'test' or data_type == 'dev':    Inputs:
    ...
    if True:
        if validate:
            ...
            if config.threshold == 'total':
                ...
                if current_results[-5] <= scores[14]:
                    best_res = [total, current_results[8], current_results[0],
                            current_results[1], current_results[9],
                            current_results[6], current_results[7],
                            current_results[10], current_results[23],
...
def update_complete_results(complete_results, avg_counter, placeholder,
    ...
    if validate:
        ...
        #****
        #if np.round(complete_results[24],3) == 0.533 and np.round(complete_results[25],3) == 0.969:
...
def final_organisation(scores, train_pred, val_pred, df, patience, epoch,
    ...
    current_main = 'main_' + str(position) + '.py'
...
def train(model, workspace_files_dir):
    ...
    learn_rate_factor = config.EXPERIMENT_DETAILS['learn_rate_factor']
    ...
    if config.Pretrain:
        ...
        if config.data_saver:
            ...
            data_saver = {}
            model, _ , optimizer = util.load_model_IDL(config.Resume_model_from, model, optimizer, learning_rate, config)
        #filter out the last output layer (the one need to be trained from scratch)
        
        #output_layer_list = ['']
        print('Resume model from {}'.format(config.Resume_model_from))
        main_logger.info('Resume model from {}'.format(config.Resume_model_from))
    else:
...
def test():
    ...
    for exp_num in range(exp_runthrough):
        ...
        if config.EXPERIMENT_DETAILS['SPLIT_BY_GENDER']:
            ...
            #scores, per_epoch 
            results_dict = evaluate_val_test(model,
                                         generator,
                                         data_type,
                                         cw[-3],
                                         np.zeros(15),
    ...
    #average and ave embedding into file
    folders = results_dict['folder']
    ...
    for folder in folders:
        for results_dict in results_dict_list:    
            for i in range(len(results_dict['folder'])):
                if results_dict['folder'][i] == folder:
                    import pdb
                    avg_emb.append(results_dict['output'][i])
        emb_output_name = str(folder)+'_IDL_emb.pickle'
        root_dir = '/data/jinhan/Dataset/daic-woz-old/audio_feats/feats_DepAudioNet/mel_snv_exp/Baseline_embedding'
        emb_output_dir = os.path.join(root_dir, emb_output_name)
        with open(emb_output_dir,'wb') as f:
            mean_emb = np.mean(np.array(avg_emb),axis=0)
            pickle.dump(mean_emb,f)
        avg_emb = []
    with open('results_dict_list.pickle','wb') as f:
...
if __name__ == '__main__':
    ...
    if args.mode == 'train':
        for i in range(exp_runthrough):
            ...
            if i == 0:
                cmd = 'cp exp_run/config_add_pretrain.py {}/mel_snv_exp/{}'.format(config.WORKSPACE_MAIN_DIR, sub_dir)
                os.system(cmd)
                cmd = 'cp exp_run/models_pytorch.py {}/mel_snv_exp/{}'.format(config.WORKSPACE_MAIN_DIR,sub_dir)
                os.system(cmd)
            train(model,
