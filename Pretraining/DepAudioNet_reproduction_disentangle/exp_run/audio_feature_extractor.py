...
def pretrain_feature_segmenter(feature,  feature_exp, dim, meta,convert_to_image=False):
    """
    Segments the features into dimensions specified by feature.shape[-1] and
    dim. The number of extra dimensions is used to create lists of the
    folder, class, score and index for this updated reshaped data array
...
def pretrain_feature_audio_segmenter(feature,feature_exp, dim, meta,audio_data,config,classifier, convert_to_image=False):
    """
    Segments the features into dimensions specified by feature.shape[-1] and
    dim. The number of extra dimensions is used to create lists of the
    folder, class, score and index for this updated reshaped data array
    ...
    #audio part
    audio_dim = config.WINDOW_SIZE + config.HOP_SIZE * (dim-1)
    #segment audio data according to featuee extra dimensions
    start = 0
    length = audio_dim
    audio_clip_list = []
    #for i in range(num_extra_dimensions):
    while start<=len(audio_data):
        audio_clip = audio_data[start: start + audio_dim]
        start = start + audio_dim - config.HOP_SIZE
        audio_clip_list.append(audio_clip)
    assert len(audio_clip_list) == num_extra_dimensions
    #extract xvector for all audio_clip
    #pdb.set_trace()
    embedding_vecs = []
    for audio_clip in audio_clip_list:
        audio_clip_tensor = torch.tensor(audio_clip)
        embeddings = classifier.encode_batch(audio_clip_tensor)
        embeddings = embeddings.cpu()
        embeddings = embeddings.numpy()[0,0,:]#get a singel vector for embeddings# fixed size 192 
        embedding_vecs.append(embeddings)
    embedding_vecs = np.array(embedding_vecs)#convert to array for convenience
    #######
    new_indexes = np.arange(num_extra_dimensions)
    #######
    if convert_to_image:
    ...
    #import pdb
    #pdb.set_trace()
    print('Finish {}'.format(new_folders[0]))
    return (new_features, new_folders, new_classes, new_scores, new_gender,
            new_indexes,embedding_vecs)
