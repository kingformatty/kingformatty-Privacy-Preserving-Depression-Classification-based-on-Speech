...
def find_weight(zeros, ones, config):
    ...
    if config.WEIGHT_TYPE == 'instance':
        ...
        #use selfdefined weights
        weights = config.EXPERIMENT_DETAILS['weight_tensor']
        instance_weights = {}
        for i in zeros:
...
def data_info(labels, mode_label, logger, config):
    ...
    if mode_label == 'train' and config.EXPERIMENT_DETAILS['SUB_SAMPLE_ND_CLASS']:
        if config.EXPERIMENT_DETAILS['Gender_min']:
        ...
        #############################
            ...
            min_set = min(len(zeros_index_f)+len(zeros_index_m), len(ones_index_f)+len(ones_index_m))
        #############################
...
def crop_sections(lengths, config):
    ...
    # dataset min
    if min_crop:
        min_crop_val = min(fndep_crop,  fdep_crop)
...
def determine_crops(features, labels, config, gender, logger, mode_label):
    ...
    if not sub_sample and use_crop and gender and not minimise_crop:
        ndep_min = min(min_samples['fndep'][0])
        dep_min = min(min_samples['fdep'][0])
...
def run_train(config, logger, checkpoint, features_dir, data_saver, val=True):
    ...
    def converge_data_to_array(file):
        data = util.converge_csv_read(file)
        print(file)
        data_array = np.array([[d[0] for d in data], [d[1] for d in data],
    ...
    import pdb
    #pdb.set_trace()
    train_labels, train_data = converge_data_to_array(train_file)
    dev_labels, dev_data = converge_data_to_array(dev_file)
    test_labels, test_data = converge_data_to_array(config.TEST_SPLIT_PATH)
    ...
    for k, i in enumerate(folder):    
        temp = np.where(train_labels[0] == i)
        if len(temp[0]) == 0:
            temp = np.where(dev_labels[0] == i)
            if len(temp[0]) == 0:
                pass
            else:
                dev_indices[temp[0][0]] = k
        else:
            train_indices[temp[0][0]] = k
#<<<<<<< Updated upstream
    import pdb
    #pdb.set_trace()
...
def run_test(config, logger, checkpoint, features_dir, data_saver,
    ...
    for k, i in enumerate(folder):
        ...
        if len(temp[0]) == 0:
            temp = np.where(dev_labels[0] == i)
            if len(temp[0]) == 0:
                temp = np.where(test_labels[0] == i)
                test_indices[temp[0][0]] = k
            else:
                dev_indices[temp[0][0]] = k
        else:
            train_indices[temp[0][0]] = k
