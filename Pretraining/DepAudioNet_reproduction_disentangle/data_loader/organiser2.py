...
from data_loader import data_gen2 as data_gen
from data_loader import data_gen2_aug as data_gen_aug
...
def get_updated_features(data, min_samples, segment_dim, freq_bins,
                         feature_exp, config, meta, mode_label='train'):
    """
    ...
    new_meta_data = afe.pretrain_feature_segmenter(reshaped_data,
                                          feature_exp,
                                          segment_dim,
                                          meta)
...
def get_updated_features_audios(data, min_samples, segment_dim, freq_bins,
                         feature_exp, config, meta, audio_data, classifier, mode_label='train'):
    """
    Final stage before segmenting the data into the form N, F, S where N is
    number of new dimensions related to the value of segment_dim. If
    'whole_train' is selected, interpolation will be applied. If the shorten
    ...
    #pdb.set_trace()
    # tuple in form of features, folders, classes, scores, genders, indices
    new_meta_data = afe.pretrain_feature_audio_segmenter(reshaped_data,
                                          feature_exp,
                                          segment_dim,
                                          meta,
                                          audio_data,
...
def process_data(freq_bins, features, labels, mode_label, min_samples, logger,
    ...
    if mode_label == 'train' and config.EXPERIMENT_DETAILS['CROP']:
        ...
        for pointer, i in enumerate(labels[0, :]):
            ...
            #                               segment_dim,
            #                               freq_bins)
            dimension = i.shape[0] // freq_bins#number of frames
            if dimension % segment_dim == 0:
                temp_length = dimension // segment_dim
            else:
                temp_length = dimension // segment_dim + 1
            length += temp_length
        #pdb.set_trace()

    update_features = np.zeros((length, freq_bins, segment_dim),
                               dtype=np.float32)
    #update_embedding_vecs = np.zeros((length, 192), dtype = np.float32)    
    ...
    #classifier = EncoderClassifier.from_hparams(source="speechbrain/spkrec-ecapa-voxceleb",savedir="/../data/jinhan/pretrained_models/spkrec-xvect-voxceleb")
    ...
    #    import pdb
        #pdb.set_trace()
        #find audio npy
    #    audio_root_path = config.WORKSPACE_MAIN_DIR+'/mel_svn_exp/audio_data/'
    #    audio_path = audio_root_path + str(meta[0]) + '_P_audio_data.npy'
    #    audio_data = np.load(audio_path)
    #    audio_data = torch.tensor(audio_data)
        #extract embedding for each utt and broadcast for loss calculation
    #    embedding = classifier.encode_batch(audio_data)
    #    embedding = embedding.cpu()
    #    embedding = embedding.numpy()[0,0,:]
    #    emb_tuple=(meta[0], embedding)
    #    #pdb.set_trace()
    #    emb_file_name = '/../data/jinhan/Dataset/librispeech/LibriSpeech360/audio_feats/feats_DepAudioNet/mel_svn_exp/embedding/'+str(meta[0])+'_emb.pickle'
    #    with open(emb_file_name, 'wb') as f:
    #        pickle.dump(emb_tuple, f)
             
        #test if feature and audio data confirm (debug purpose)
    #    print('Audio Length: {} samples'.format(len(audio_data)))
    #    print('Data has spec {} temporal value'.format(data.shape[0]/40))
    #    print('Derived Audio Length from spec: {} samples'.format(data.shape[0]/40*512))
    #    diff = np.abs(len(audio_data) - data.shape[0]/40 * 512)
    ...
    #    assert diff <= 512
        ...
        #(new_features, new_folders, new_classes, new_scores, new_genders,
        # new_indices, embedding_vecs) = get_updated_features_audios(data,
        #                                     min_samples,
        #                                     segment_dim,
        #                                     freq_bins,
        #                                     feature_exp,
        ...
        #update_embedding_vecs[pointer: pointer + z,:] = embedding_vecs
        #pdb.set_trace()
    ...
    return update_features, update_labels, locator#, update_embedding_vces
...
def determine_crops(features, labels, config, gender, logger, mode_label):
    ...
    for i in range(features.shape[0]):
        lengths.append(len(features[i][0]))
    #pdb.set_trace()
...
def run_train(config, logger, checkpoint, features_dir, data_saver):
    ...
    if config.EXPERIMENT_DETAILS['Augmentation']:
        #pdb.set_trace()
        _,_,summary_file_aug, database_aug = file_paths(config.AUG_DATA_path,config)
    #elif config.EXPERIMENT_DETAILS['Augmentation'] is not None:
    #    _,_,summary_file_aug, database_aug = file_paths(config.AUG_DATA_path,config)
    #TODO
    #VTLP
    #elif config.EXPERIMENT_DETAILS['Augmentation'] == 'VTLP':
    ...
    #convert the first entry ini each element into float
...
#>>>>>>> Stashed changes
    ...
    train_labels_orig = np.copy(train_labels)
    dev_labels_orig = np.copy(dev_labels)
    #pdb.set_trace()
    ...
    #pdb.set_trace()
    if config.EXPERIMENT_DETAILS['Augmentation']:
        dev_features_aug, _,_,_,_ = organise_data(config,
                                                    logger,
                                                    dev_labels_orig,
                                                    database_aug,
                                                    mode_label='dev')
    ...
    elif config.EXPERIMENT_DETAILS['Augmentation']:
        generator = data_gen_aug.GenerateData(train_labels=train_labels,
                                      dev_labels=dev_labels,
                                      train_feat=train_features,
                                      train_feat_aug = train_features_aug,
                                      #train_emb = train_emb,
                                      #train_emb_aug = train_emg_aug,
                                      dev_feat=dev_features,
                                      dev_feat_aug = dev_features_aug,
                                      #dev_emb = dev_emb,
                                      #dev_emb_aug = dev_emb_aug,
                                      train_loc=train_loc,
                                      dev_loc=dev_loc,
                                      train_indices=train_index,
                                      dev_indices=dev_index,
