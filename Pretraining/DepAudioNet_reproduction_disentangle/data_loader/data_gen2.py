...
class GenerateData:
    ...
    def generate_train_data(self, epoch):
        ...
        while True:
            ...
            if self.gender_balance:
                ...
                #batch_folders = train_folders[current_indices.tolist()]
                ###################################
            batch_data = self.train_feat[current_indices]# N(20) * H(40)  * W(120) 
            ...
            for folder in batch_folders:
                emb_path = self.config.SPK_EMB_DIR+str(folder)+'_emb.pickle'
                _, emb = pickle.load(open(emb_path,'rb'))
                batch_emb.append(emb)
            batch_emb = np.array(batch_emb)#N * 192
            #######
            yield (batch_data, batch_labels, batch_folders, epoch, reset), \
                  locs_array, self.data_saver, batch_emb
    ...
    def generate_development_data(self, epoch):
        ...
        for i in range(self.BN_dev):
            ...
            batch_indices = []
            import pdb
            if self.config.EXPERIMENT_DETAILS['random_speaker'] or self.batch_size > len(self.dev_loc):
            ...
            #######
            locs_array = 0
            yield batch_data, batch_labels, batch_folders, locs_array, batch_emb
