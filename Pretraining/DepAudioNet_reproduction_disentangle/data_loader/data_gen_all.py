...
class GenerateData:
    ...
    def generate_train_data(self, epoch):
        ...
        indices = np.concatenate((train_indices_zeros, train_indices_ones),axis = 0).astype(int)
        indices = indices.tolist()
        random.shuffle(indices) 
        """
        while True:
            reset = False
            pointer_end = pointer + self.batch_size
            import pdb
            if pointer_end >= len(indices):
                epoch += 1
                print(pointer)
                pointer = 0
                reset = True
                if epoch == self.config.EXPERIMENT_DETAILS['TOTAL_EPOCHS']:
                    break
            batch_indices = indices[pointer: pointer+ self.batch_size]
            batch_data = self.train_feat[batch_indices]
            batch_labels = train_classes[batch_indices]
            batch_folders = train_folders[batch_indices]
            self.data_saver = {'mean': self.mean,
                               'std': self.standard_deviation}
            batch_data = util.normalise(batch_data, self.mean, self.standard_deviation)
            pointer += self.batch_size
            locs_array = 0#np.ones((batch_data.shape[0]),dtype = np.int)
            #pdb.set_trace()
            yield (batch_data, batch_labels, batch_folders, epoch, reset), locs_array, self.data_saver
