...
class GenerateData:
    def __init__(self, train_labels, dev_labels, train_feat,train_feat_aug,  dev_feat,dev_feat_aug, train_loc, dev_loc, train_indices, dev_indices, logger, config, checkpoint, gender_balance=False, data_saver=None):
        """
        ...
        self.train_feat_aug = train_feat_aug
        #self.train_emb = train_emb
        #self.train_emb_aug = train_emb_aug
        self.dev_feat = dev_feat
        self.dev_feat_aug = dev_feat_aug
        #self.dev_emb = dev_emb
        #self.dev_emb_aug = dev_emb_aug
        self.train_loc = train_loc
        self.dev_loc = dev_loc
        self.logger = logger
        self.checkpoint = checkpoint
        self.data_saver = data_saver
        self.BN_train = self.config.EXPERIMENT_DETAILS['BN_per_epoch_train']
        self.BN_dev = self.config.EXPERIMENT_DETAILS['BN_dev']
        #pdb.set_trace()
        ...
        if len(data_saver) > 0:
            ...
            stats_aug = self.train_feat_aug[for_stats]
            self.mean_aug, self.standard_deviation_aug = self.calculate_stats(stats_aug)
    ...
    def generate_train_data(self, epoch):
        ...
        while True:
            ...
            if self.gender_balance:
                ...
                import pdb 
                ########################
                #generate batch indices, with audios from distinct speaker
                batch_indices = []
                if self.config.EXPERIMENT_DETAILS['random_speaker']:
                    batch_indices = np.random.choice(np.arange(self.train_loc[-1][1]),size = self.batch_size)
                    import pdb
                else:    
                    chosen_folder_indices = np.random.choice(np.arange(len(self.train_loc)),replace = False, size = self.batch_size)
                    for index in chosen_folder_indices:
                        folder_segments_id = self.train_loc[index]
                        target_segment_id = np.random.choice(np.arange(folder_segments_id[0],folder_segments_id[1]),size = 1)
                        batch_indices.append(target_segment_id[0])
                current_indices = batch_indices
                batch_labels = train_classes[current_indices]
                batch_folders = train_folders[current_indices]
                batch_counter += 1
                if batch_counter >= self.BN_train: #set number of batch with in an epoch, since random, not fixed
                    epoch += 1
                    reset = True
                    batch_counter = 0
                    import pdb
                self.data_saver = {'mean': self.mean,
                                    'std': self.standard_deviation,
                                   'mean_aug':self.mean_aug,
                                   'std_aug':self.standard_deviation_aug}
                ########################
            ...
            batch_data_aug = self.train_feat_aug[current_indices]
            batch_emb = self.train_emb[current_indices]
            batch_emb_aug = self.train_emb_aug[current_indices]
            locs_array = 0
            batch_data = util.normalise(batch_data, self.mean,
                                        self.standard_deviation)
            batch_data_aug = util.normalise(batch_data_aug, self.mean_aug,
                                            self.standard_deviation_aug)

            yield (batch_data, batch_labels, batch_folders, epoch, reset, batch_data_aug), \
                  locs_array, self.data_saver, (batch_emb, batch_emb_aug)
    ...
    def generate_development_data(self, epoch):
        ...
        '''
        batch_counter = 0
        for i in range(self.BN_dev):
            import pdb
            batch_indices = []
            if self.config.EXPERIMENT_DETAILS['random_speaker']:
                batch_indices = np.random.choice(np.arange(self.dev_loc[-1][1]),size = self.batch_size)
            else:               
                chosen_folder_indices = np.random.choice(np.arange(25),replace = False, size = 20)
                for index in chosen_folder_indices:
                    folder_segments_id = self.dev_loc[index]
                    target_segment_id = np.random.choice(np.arange(folder_segments_id[0],folder_segments_id[1]),size = 1)
                    batch_indices.append(target_segment_id[0])
            current_indices = batch_indices
            batch_labels = classes[current_indices]
            batch_folders = folders[current_indices]
            batch_counter += 1
            batch_data = self.dev_feat[current_indices]
            batch_data_aug = self.dev_feat_aug[current_indices]
            batch_emb = self.dev_emb[current_indices]
            batch_emb_aug = self.dev_emb_aug[current_indices]
            batch_data = util.normalise(batch_data,self.mean,self.standard_deviation)
            batch_data_aug = util.normalise(batch_data_aug, self.mean_aug, self.standard_deviation_aug)
            locs_array = 0
            yield batch_data, batch_labels, batch_folders, locs_array, batch_data_aug, batch_emb, batch_emg_aug
