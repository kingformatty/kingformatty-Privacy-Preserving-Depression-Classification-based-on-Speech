...
from utilities.mi_estimators import CLUBSample_group
...
def get_output_from_model(model, data, cuda=True):
    """
    ...
    output,output_seq = model(current_data)
    #if cuda:
    #    output = output.cpu()
...
def train(model, workspace_files_dir):
    ...
    #######spk_related
    model_MI = CLUBSample_group(192,192,256) #Dimension Check 
    if cuda:
        model_MI.cuda()
    optimizer_MI = torch.optim.Adam(model_MI.parameters(), lr=config.lr_MI)
    ...
    for (iteration, (batch, train_batch_loc, data_saver, batch_emb)) in enumerate(
        ...
        batch_emb = torch.LongTensor(batch_emb)
        #batch_output,batch_output_seq = get_output_from_model(model=model,
        #                                     data=batch[0])
        ...
        if config.use_spec_aug:
            ...
            batch_output_aug, batch_output_aug_seq = get_output_from_model(model=model,
                                                     data = aug_data)
        ...
        for j in range(config.iter_MI):
            if cuda:
                batch_emb = batch_emb.cuda()
            optimizer_MI, loss_MI = mu.mi_first_forward(model, orig_data, batch_emb,optimizer_MI, model_MI,  config)
            #print('MI_loss: {}'.format(loss_MI.item()))
        import pdb
        batch_output,batch_output_seq = get_output_from_model(model=model,
                                             data=orig_data)
        train_loss,loss_MI = mu.calculate_IDL_mutual_spk_loss(batch_output,
                                           batch_output_aug,
                                           batch_output_seq,
                                           batch_emb,
                                           model_MI,
        ...
        #if iteration % 20 == 0:
        if iteration % 50 == 0:
            print('Epoch: {}, iteration: {}, log_likelihood: {}, MI_loss: {}'.format(current_epoch,iteration, log_like, loss_MI.item())) 
            ...
            if validate:
                ...
                for (dev_iter,data) in enumerate(generate_dev):
                    if not config.EXPERIMENT_DETAILS['Augmentation']:
                        (batch_data, batch_label, batch_folder, batch_locatori, batch_emb) = data
                    else:
                    ...
                    with torch.no_grad():
                        ...
                        if config.use_spec_aug:
                            for i in range(aug_data.shape[0]):
                                ...
                                aug_data_i = spec_aug(orig_data_i,config)
                                aug_data[i,:,:] = np.transpose(aug_data_i)
                            batch_output_aug, batch_output_aug_seq = get_output_from_model(model = model, data = aug_data)
                        elif config.use_time_mask:
                            for i in range(aug_data.shape[0]):
                                mask_start = np.random.randint(aug_data.shape[2])
                        ...
                        optimizer_MI, loss_MI = mu.mi_first_forward_eval(model, orig_data, batch_emb,optimizer_MI, model_MI,  config)
                        train_loss, loss_MI = mu.calculate_IDL_mutual_spk_loss(batch_output,
                                                           batch_output_aug,
                                                           batch_output_seq,
                                                           batch_emb,
                ...
                avg_loss_MI = loss_MI_sum.item() / (dev_iter+1)
