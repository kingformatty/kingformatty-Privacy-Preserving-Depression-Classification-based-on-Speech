...
import os
import time
from data_loader import organiser#_all as organiser
import shutil
import sys
import torch
import random
...
from exp_run.models_pytorch import PretrainDAN_Apply as PretrainDAN_Apply
from exp_run.models_pytorch import new_model_1, new_model_2, new_model_3, new_model_4_stat_pool_LSTM, new_model_5_stat_pool_BLSTM, new_model_6_stat_pool_var_LSTM, new_model_7_stat_pool_var_BLSTM, new_model_general, new_model_general_freeze_fx
from exp_run import config_add_pretrain as config
from pytorch_model_summary import summary
learn_rate_factor = 2
EPS = 1e-12

def evaluation_for_test(results_dict, num_class, f_score, main_logger, prediction_metric):
    import pdb
    #pdb.set_trace()
    ...
    if prediction_metric == '0':
        f_score_avg = []
        temp_out = np.zeros((exp_runthrough, len(results_dict['output'].keys())))
        ...
        for exp in range(exp_runthrough):
            tmp_scores[exp,:], _ = prediction_and_accuracy(temp_out[exp,:],
                                                            temp_tar,
                                                             True,
                                                              num_class,
                                                               np.zeros(15),
                                                                0,
                                                                 0,
                                                                  f_score)
            f_score_avg = f_score_avg + [np.mean(temp_scores[exo,6:8])]
    ...
    #average performance over all models
        ...
        for f in results_dict['output'].keys():
            final_results[f] = np.average(results_dict['output'][f] / results_dict['accum'][f])
        import pdb
        #pdb.set_trace()
        temp_out = np.array(list(final_results.values()))
        scores, _ = prediction_and_accuracy(temp_out, temp_tar, True, num_class, np.zeros(15), 0, 0, f_score)

    #calculate majority vote from all models
    elif prediction_metric == '2':
        print('\n Performing majority vote on accumulated predictions')
        for f in results_dict['output'].keys():
            final_results[f] = np.average(np.round(results_dict['output'][f] / results_dict['accum'][f]))
        temp_out = np.array(list(final_results.values()))
        scores, _ = prediction_and_accuracy(temp_out, temp_tar, True, num_class, np.zeros(15), 0, 0, f_score)
    
    scores[8] = np.mean(scores[0:2])
    scores[9] = np.mean(scores[6:8])
    scores = [scores[8], scores[0], scores[1],scores[2], scores[3], scores[4], scores[5], scores[9], scores[6], scores[7], scores[11], scores[12], scores[13], scores[14]]
    return scores
...
def evaluate(model, generator, data_type, class_weights, comp_res, comp_res_seg,class_num,
             f_score_average, f_score_average_seg, epochs, logger, gender_balance=False):
    """
    ...
    for p, fol in enumerate(folders):
        if fol not in collected_output.keys():
            ...
            counter[fol] += 1
    #pdb.set_trace() 
    #segment-wise evaluation, just for reference
    complete_results_seg, _ = prediction_and_accuracy(outputs, targets, True, class_num, comp_res_seg,0, f_score_average_seg)
    if mode == 'test':
    ...
    #pdb.set_trace()
    if data_type == 'test':
        return complete_results, (per_epoch_pred, intermediate_outputs, intermediate_counter, intermediate_targets), complete_results_seg
    else:
        return complete_results, per_epoch_pred, complete_results_seg
    ...
    if mode == 'test':
        if data_type == 'test':
            ...
            log_path = os.path.join(current_dir, 'log',
                                    f"model_test.log")
            #if config.test_mode == 'avg':
            #   log_path = os.path.join(current_dir,'log',
                                    #f"model_test_avg.log")
    else:
        log_path = os.path.join(current_dir, 'log',
...
def create_model():
    ...
    if config.EXPERIMENT_DETAILS['FEATURE_EXP'] == 'mel':
        ...
        if config.EXPERIMENT_DETAILS['model_option'] == 1:
            ...
            model = new_model_4_stat_pool_LSTM()
        elif config.EXPERIMENT_DETAILS['model_option'] == 5:
            model = new_model_5_stat_pool_BLSTM()
        elif config.EXPERIMENT_DETAILS['model_option'] == 6:
            model = new_model_6_stat_pool_var_LSTM()
        elif config.EXPERIMENT_DETAILS['model_option'] == 7:
            model = new_model_7_stat_pool_var_BLSTM()
        elif config.EXPERIMENT_DETAILS['model_option'] == 0:
            model = CustomMel()
    """
    if config.EXPERIMENT_DETAILS['FEATURE_EXP'] == 'mel':
        if config.EXPERIMENT_DETAILS['custommel']:
            model = CustomMel()
        else:
            if config.freeze:
                model = new_model_general_freeze_fx(config)
            else: 
                model = new_model_general(config)
...
def record_top_results(current_results, scores, epoch):
    ...
    '''
    #use self-defined score as threshold for best score
    ...
    import pdb
    #pdb.set_trace()    
    #if current_results[-5] < scores[14]: #best loss
    #if current_results[-6] > scores[11]: #best fscore criterion
    if config.threshold == 'total':
        if total >= scores[0]: #best total score criterion
            best_res = [total, current_results[8], current_results[0],
                    current_results[1], current_results[9],
                    current_results[6], current_results[7],
                    current_results[10], current_results[23],
            ...
            best_res = scores
    elif config.threshold == 'fscore':
        if current_results[-6] >= scores[11]: #best total score criterion
            best_res = [total, current_results[8], current_results[0],
                    current_results[1], current_results[9],
                    current_results[6], current_results[7],
                    current_results[10], current_results[23],
            ...
            best_res = scores
    elif config.threshold == 'loss':
        if current_results[-5] <= scores[14]: #best total score criterion
            best_res = [total, current_results[8], current_results[0],
                    current_results[1], current_results[9],
                    current_results[6], current_results[7],
                    current_results[10], current_results[23],
...
def update_complete_results(complete_results, complete_results_seg, avg_counter, placeholder,
                            best_scores):
    """
    ...
    complete_results_seg[23] = np.mean(complete_results_seg[15:17])
    complete_results_seg[24] = np.mean(complete_results_seg[21:23])

    
    print_log_results(placeholder, complete_results[0:15], 'train')
    print_log_results(placeholder, complete_results[15:], 'dev')
    print_log_results(placeholder, complete_results_seg[15:],'dev:segment')
    import pdb
    best_scores = record_top_results(complete_results,
                                     best_scores,
                                     placeholder)

    return complete_results, best_scores, complete_results_seg
...
def print_log_results(epoch, results, data_type):
    ...
    if data_type != 'dev:segment':
        ...
        print('Segment-wise validation set performance')
        print('dev:segment Accuracy: Mean: {} - [{},{}], F1_Score: Mean: {}, [{},{}]'.format(np.round(results[8],3),np.round(results[0],3),np.round(results[1],3),np.round(results[9],3),np.round(results[6],3),np.round(results[7],3)))
    ...
    if data_type != 'dev:segment':
        ...
        main_logger.info('Segment-wise validation set performance')
        main_logger.info('dev:segment Accuracy: Mean: {} - [{},{}], F1_Score: Mean: {}, [{},{}]'.format(np.round(results[8],3),np.round(results[0],3),np.round(results[1],3),np.round(results[9],3),np.round(results[6],3),np.round(results[7],3)))
...
def bookeeping():
    ...
    for f in range(len(files)):
        ...
        if data[0] == 0:
            ...
            import pdb
            #pdb.set_trace()
            #here only the model with best score will be saved, otherwise, delete
            ...
            #delete file or not
            os.mkdir(os.path.join(current_model_dir,'backup_mod'))
            for m in mod_paths:
                #os.remove(m)
                shutil.move(m,current_model_dir+'/backup_mod/')
...
def train(model, workspace_files_dir):
    ...
    if config.Pretrain:
        ...
        if config.data_saver:
            ...
            model, data_saver = util.load_model_IDL(config.Resume_model_from,
                                               model,
                                               optimizer,
                                               config)
        else:
            data_saver = {}
            model, _ = util.load_model_IDL(config.Resume_model_from,
                                                model,
                                                optimizer,
                                                config)
    else:
    ...
    complete_results_seg = np.zeros(30)
    ...
    for (iteration, (batch, train_batch_loc, data_saver)) in enumerate(
        ...
        import pdb
        #cw = config.EXPERIMENT_DETAILS['weight_tensor']
        ...
        if begin_evaluate:
            ...
            if validate:
                ...
                import pdb
                (complete_results[15:], per_epoch_val_pred, complete_results_seg[15:]) = evaluate(
                    model=model,
                    generator=gen,
                    data_type='dev',
                    class_weights=train_info[-1],
                    comp_res=complete_results[15:],
                    comp_res_seg = complete_results_seg[15:],
                    class_num=num_of_classes,
                    f_score_average=None,
                    f_score_average_seg = None,
                    epochs=start_epoch,
                    logger=main_logger,
                    gender_balance=gender_balance)
                import pdb
                complete_results, best_scores,complete_results_seg = \
                    update_complete_results(complete_results,
                                            complete_results_seg,
                                            avg_counter,
                                            placeholder,
                                            best_scores)
                avg_counter = 0
...
def test():
    ...
    if config.EXPERIMENT_DETAILS['SPLIT_BY_GENDER']:
        ...
        comp_scores_seg = np.zeros((exp_runthrough,14))
    
    if data_type == 'test':
        results = {'output': {},'target': {}, 'accum': {}}
    ####
    p_r_f_score_list = []
    p_r_f_score_list_seg = []
    ...
    for exp_num in range(exp_runthrough):
        ...
        for file in os.listdir(model_dir):
            if file.endswith(".pth"):
                #if file.find(config.test_epoch) != -1:
        ...
        _, data_saver = util.load_model(checkpoint_path=model_dir,
                                        model=model,
                                        optimizer=optimizer)
        #data_saver = util.load_model_outputs(model_dir,'test')
        import pdb
        #pdb.set_trace()
        ...
        if config.EXPERIMENT_DETAILS['SPLIT_BY_GENDER']:
            ...
            scores, per_epoch, scores_seg = evaluate(model,
                                         generator,
                                         data_type,
                                         cw[-3],
                                         np.zeros(30),
            ...
            scores_seg[8] = np.mean(scores_seg[0:2])
            scores_seg[9] = np.mean(scores_seg[6:8])
            scores_seg = [scores_seg[8], scores_seg[0], scores_seg[1], scores_seg[2], 
                      scores_seg[3], scores_seg[4], scores_seg[5],scores_seg[9],
                      scores_seg[6], scores_seg[7], scores_seg[11], scores_seg[12],
                      scores_seg[13], scores_seg[14]]

            
            print("Scores: ", scores)
            print('Scores_seg: ', scores_seg)
            comp_scores[exp_num, :] = scores
            comp_scores_seg[exp_num,:] = scores_seg
            main_logger.info(f"Scores are: \n{scores}")
            main_logger.info(f'Scores_seg are: \n{scores_seg}')
            p_r_f_score_list.append(scores[0:10])
            p_r_f_score_list_seg.append(scores_seg[0:10])
            import pdb
            #pdb.set_trace() 
            if data_type == 'test':
               per_epoch, outputs, accum, targets = per_epoch# outputs: number of 1, accum: total number
               for counter_id, folder in enumerate(outputs):
                    if folder not in results['output'].keys():
                        ...
                        results['output'][folder] = np.append(results['output'][folder], outputs[folder][0])
            #pdb.set_trace()
    if data_type == 'dev':
    # tn_fp_fn_tp
        print('*'*50)
        print('Utterance-wise')
        main_logger.info('Utterance-wise')
        print('Average:')
        ...
        main_logger.info('best p_r_fscore among all model (according to best fscore) is')
        model_f_list = [x[-3] for x in p_r_f_score_list]
        model_f_list = np.array(model_f_list)
        best_model_id = np.argmax(model_f_list)
        best_model_p_r_f = p_r_f_score_list[best_model_id]
        main_logger.info('Acc(D): {}\nPrecision(D): {}\nRecall(D): {}\nF1(D): {}\nAcc(ND): {}\nPrecision(ND): {}\nRecall(ND): {}\n F1(ND): {}\n F1(Avg):{}\n Acc(Avg): {}'.format(best_model_p_r_f[2],best_model_p_r_f[4], best_model_p_r_f[6],best_model_p_r_f[9],best_model_p_r_f[1],best_model_p_r_f[3],best_model_p_r_f[5], best_model_p_r_f[8],best_model_p_r_f[7],best_model_p_r_f[0]))
        print('Best p_r_Fscore among all model (according to best fscore) is')
        print('Acc(D): {}\nPrecision(D): {}\nRecall(D): {}\nF1(D): {}\nAcc(ND): {}\nPrecision(ND): {}\nRecall(ND): {}\n F1(ND): {}\n F1(Avg):{}\n Acc(Avg): {}'.format(best_model_p_r_f[2],best_model_p_r_f[4], best_model_p_r_f[6],best_model_p_r_f[9],best_model_p_r_f[1],best_model_p_r_f[3],best_model_p_r_f[5], best_model_p_r_f[8],best_model_p_r_f[7],best_model_p_r_f[0]))
        ...
        print('Acc(D): {}\nPrecision(D): {}\nRecall(D): {}\nF1(D): {}\nAcc(ND): {}\nPrecision(ND): {}\nRecall(ND): {}\n F1(ND): {}\n F1(Avg):{}\n Acc(Avg): {}'.format(p_r_f_score_list[2],p_r_f_score_list[4], p_r_f_score_list[6],p_r_f_score_list[9],p_r_f_score_list[1],p_r_f_score_list[3],p_r_f_score_list[5], p_r_f_score_list[8],p_r_f_score_list[7],p_r_f_score_list[0]))
        main_logger.info('Acc(D): {}\nPrecision(D): {}\nRecall(D): {}\nF1(D): {}\nAcc(ND): {}\nPrecision(ND): {}\nRecall(ND): {}\n F1(ND): {}\n F1(Avg):{}\n Acc(Avg): {}'.format(p_r_f_score_list[2],p_r_f_score_list[4], p_r_f_score_list[6],p_r_f_score_list[9],p_r_f_score_list[1],p_r_f_score_list[3],p_r_f_score_list[5], p_r_f_score_list[8],p_r_f_score_list[7],p_r_f_score_list[0]))
        ...
        print('*'*50)
        print('Segment-wise')
        main_logger.info('Segment-wise')
        print('Average:')
        comp_scores_seg_avg = np.mean(comp_scores_seg, axis=0)
        print(comp_scores_seg_avg)
        main_logger.info('Best p_r_Fscores for each model are')
        main_logger.info(p_r_f_score_list_seg)
        print('Best p_r_f for each model:')
        print(p_r_f_score_list_seg)
        best_model_p_r_f = p_r_f_score_list_seg[best_model_id]
        main_logger.info('best p_r_fscore among all model (according to best fscore) is')
        main_logger.info('Acc(D): {}\nPrecision(D): {}\nRecall(D): {}\nF1(D): {}\nAcc(ND): {}\nPrecision(ND): {}\nRecall(ND): {}\n F1(ND): {}\n F1(Avg):{}\n Acc(Avg): {}'.format(best_model_p_r_f[2],best_model_p_r_f[4], best_model_p_r_f[6],best_model_p_r_f[9],best_model_p_r_f[1],best_model_p_r_f[3],best_model_p_r_f[5], best_model_p_r_f[8],best_model_p_r_f[7],best_model_p_r_f[0]))
        print('Best p_r_Fscore among all model (according to best fscore) is')
        print('Acc(D): {}\nPrecision(D): {}\nRecall(D): {}\nF1(D): {}\nAcc(ND): {}\nPrecision(ND): {}\nRecall(ND): {}\n F1(ND): {}\n F1(Avg):{}\n Acc(Avg): {}'.format(best_model_p_r_f[2],best_model_p_r_f[4], best_model_p_r_f[6],best_model_p_r_f[9],best_model_p_r_f[1],best_model_p_r_f[3],best_model_p_r_f[5], best_model_p_r_f[8],best_model_p_r_f[7],best_model_p_r_f[0]))
        ...
        main_logger.info('Average p_r_Fscore for all models is')
        main_logger.info(np.mean(p_r_f_score_list_seg,axis = 0))
        print('Average p_r_f for all models')
        print(np.mean(p_r_f_score_list_seg, axis=0))
        p_r_f_score_list = np.mean(p_r_f_score_list_seg, axis=0)
    ...
    # tn_fp_fn_tp
        #average
        comp_scores_offical = evaluation_for_test(results, num_of_classes, f_score, main_logger,'1')
        import pdb
        #pdb.set_trace()
        ...
        print('*'*50)
        print('Offical Eva')
        main_logger.info('Offical Eva')
        main_logger.info('Prediction_metric: Average')
        print('Prediction_metric: Average')
        p_r_f_score_list = comp_scores_offical[:10]
        print('Acc(D): {}\nPrecision(D): {}\nRecall(D): {}\nF1(D): {}\nAcc(ND): {}\nPrecision(ND): {}\nRecall(ND): {}\n F1(ND): {}\n F1(Avg):{}\n Acc(Avg): {}'.format(p_r_f_score_list[2],p_r_f_score_list[4], p_r_f_score_list[6],p_r_f_score_list[9],p_r_f_score_list[1],p_r_f_score_list[3],p_r_f_score_list[5], p_r_f_score_list[8],p_r_f_score_list[7],p_r_f_score_list[0]))
        main_logger.info('Acc(D): {}\nPrecision(D): {}\nRecall(D): {}\nF1(D): {}\nAcc(ND): {}\nPrecision(ND): {}\nRecall(ND): {}\n F1(ND): {}\n F1(Avg):{}\n Acc(Avg): {}'.format(p_r_f_score_list[2],p_r_f_score_list[4], p_r_f_score_list[6],p_r_f_score_list[9],p_r_f_score_list[1],p_r_f_score_list[3],p_r_f_score_list[5], p_r_f_score_list[8],p_r_f_score_list[7],p_r_f_score_list[0]))
        #main_logger.info(f"Average Scores: \n{comp_scores_avg}")
        ...
        print('*'*50)
        prediction_metric = '2' 
        comp_scores_offical = evaluation_for_test(results, num_of_classes, f_score, main_logger,'2')
        print('Offical Eva')
        main_logger.info('Offical Eva')
        main_logger.info('Prediction_metric: Majority Voting')
        print('Prediction_metric: Majority Voting')
        p_r_f_score_list = comp_scores_offical[:10]
        print('Acc(D): {}\nPrecision(D): {}\nRecall(D): {}\nF1(D): {}\nAcc(ND): {}\nPrecision(ND): {}\nRecall(ND): {}\n F1(ND): {}\n F1(Avg):{}\n Acc(Avg): {}'.format(p_r_f_score_list[2],p_r_f_score_list[4], p_r_f_score_list[6],p_r_f_score_list[9],p_r_f_score_list[1],p_r_f_score_list[3],p_r_f_score_list[5], p_r_f_score_list[8],p_r_f_score_list[7],p_r_f_score_list[0]))
        main_logger.info('Acc(D): {}\nPrecision(D): {}\nRecall(D): {}\nF1(D): {}\nAcc(ND): {}\nPrecision(ND): {}\nRecall(ND): {}\n F1(ND): {}\n F1(Avg):{}\n Acc(Avg): {}'.format(p_r_f_score_list[2],p_r_f_score_list[4], p_r_f_score_list[6],p_r_f_score_list[9],p_r_f_score_list[1],p_r_f_score_list[3],p_r_f_score_list[5], p_r_f_score_list[8],p_r_f_score_list[7],p_r_f_score_list[0]))
        
        '''
    #model fusion when test_mode is avg
    #fuse model
    """
    import pdb
    cmd = 'exp_run/average_checkpoint.py --exp_dir {} --out_name {} --num {} '.format(current_dir,config.out_name,config.num) 
    os.system(cmd) 
    if config.EXPERIMENT_DETAILS['SPLIT_BY_GENDER']:
        comp_scores = np.zeros((exp_runthrough, 20))
    else:
    ...
    if config.test_mode == 'avg':
        ...
        if config.EXPERIMENT_DETAILS['SPLIT_BY_GENDER']:
            ...
            print("Scores: ", scores)
            print('Scores_seg: ', scores_seg)
            comp_scores[0, :] = scores
            comp_scores_seg[0,:] = scores_seg
            main_logger.info(f"Scores are: \n{scores}")
            main_logger.info(f'Scores_seg are: \n{scores_seg}')
            p_r_f_score_list.append(scores[0:10])
            p_r_f_score_list_seg.append(scores_seg[0:10])
    ...
    main_logger.info('best p_r_fscore among all model (according to best fscore) is')
    best_model_p_r_f = p_r_f_score_list_seg[best_model_id]
    main_logger.info('Precision(D): {}\nRecall(D): {}\nF1(D): {}\nPrecision(ND): {}\nRecall(ND): {}\n F1(ND): {}\n F1(Avg):{}'.format(best_model_p_r_f[1], best_model_p_r_f[3],best_model_p_r_f[6],best_model_p_r_f[0],best_model_p_r_f[2], best_model_p_r_f[5],best_model_p_r_f[4]))
    print('Best p_r_Fscore among all model (according to best fscore) is')
    print('Precision(D): {}\nRecall(D): {}\nF1(D): {}\nPrecision(ND): {}\nRecall(ND): {}\n F1(ND): {}\n F1(Avg):{}'.format(best_model_p_r_f[1], best_model_p_r_f[3],best_model_p_r_f[6],best_model_p_r_f[0],best_model_p_r_f[2], best_model_p_r_f[5],best_model_p_r_f[4]))
    main_logger.info('Average p_r_Fscore for all models is')
    main_logger.info(np.mean(p_r_f_score_list_seg,axis = 0))
    ...
    if args.mode == 'train':
        for i in range(exp_runthrough):
            ...
            if i == 0:
                ...
                cmd = 'cp exp_run/config_add_pretrain.py {}'.format(sub_dir)
                os.system(cmd)
                #copy model file
                cmd = 'cp exp_run/models_pytorch.py {}'.format(sub_dir) 
                os.system(cmd)
            train(model,
