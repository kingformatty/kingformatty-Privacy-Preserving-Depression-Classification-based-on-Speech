...
def freq_mask(x, F=30, n_mask=2, replace_with_zero=True, mode='random'):
...
def time_mask(spec, mean, T=40, n_mask=2, replace_with_zero=True, replace_with_mean = True, mode='random'):
    """freq mask for spec agument
    ...
    if mode == 'fix':
        ts = [T for i in range(n_mask)]
    if mode == 'random':
        if isinstance(T, float):
            T = int(len_spectro * T)
        ts = numpy.random.randint(0, T, size=(n_mask))
    ...
    for t in ts:
        ...
        if replace_with_zero:
            ...
            cloned[t_zero:t_zero+t] = mean.T
        else:
            cloned[t_zero:t_zero+t] = cloned.mean()
    return cloned


def spec_aug(x, args, mean):
    """spec agument
    ...
    x = time_mask(
        ...
        args.n_time_mask,
        replace_with_zero=args.replace_with_zero,
        replace_with_mean=args.replace_with_mean,
        mode=args.mode,
    )
    return x
