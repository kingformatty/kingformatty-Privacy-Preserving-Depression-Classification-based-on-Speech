...
def save_model(epoch_iter, model, optimizer, main_logger, model_dir,
    ...
    #save_out_path = model_dir + '/data_saver_'+str(epoch_iter)+'.pickle'
    with open(save_out_path, 'wb') as f:
        pickle.dump(data_saver, f)
    
def save_model_Pretrain(epoch_iter, model, optimizer, main_logger, model_dir,
               data_saver,config):
    """
    Saves the model weights along with the current epoch and all the random
    states that are used during the experiment. Also saves the current state
    ...
    save_out_dict = {'epoch': epoch_iter,
                     ...
                     'numpy_rng_state': np.random.get_state(),
                     'random_rng_state': random.getstate()}
    if epoch_iter >= config.EXPERIMENT_DETAILS['SAVE_MODEL_FROM']:
        save_out_path = os.path.join(model_dir,
                                 f"md_{epoch_iter}_epochs.pth")
...
def load_model_IDL(checkpoint_path, model, optimizer,config):
    """
    Loads the model weights along with the current epoch and all the random
    states that are used during the experiment. Also loads the current state
    of the data loader for continuity
    ...
    for name, param in model.named_parameters():
        ...
        if  config.batch_norm:
            ...
            if name in pretrained_name and name.find('bn')==-1:
                if param.shape == pretrained_dict[name].shape:
                    param.data.copy_(pretrained_dict[name])
                if config.freeze:
                    param.requires_grad = False
    ...
    #np.random.set_state(checkpoint['numpy_rng_state'])
    #random.setstate(checkpoint['random_rng_state'])
    data_saver_name = 'data_saver_'+str(epoch)+'.pickle' #pickle
    data_saver_path = checkpoint_path.split('/')
    data_saver_path = checkpoint_path.replace(data_saver_path[-1],
                                              data_saver_name)
    with open(data_saver_path, 'rb') as f:
...
def load_model_outputs(model_dir,data_mode = 'train'):
    ...
    if data_mode != 'train':
        data_saver_path = model_dir.replace(model_dir.split('/')[-1],'data_saver.pickle')
...
def converge_csv_read(file, start = None, end = None):
    data = []
    audio_paths = []
    all_converge_df = pd.read_csv(file, sep = ';',header = 'infer')
    ...
    for filename in audio_paths:
        ...
        pid = int(spkid)
        clss = 1 if filename.split('/')[-3] == 'cases' else 0
        scre = -1
        gender = 0
        row = [pid,clss,scre,gender]
        data.append(row)
    return data
